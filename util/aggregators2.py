...
class Aggregator(object):
    ...
    def _mix_neighbor_vectors(self, self_vectors, neighbor_vectors):
        avg = False
        if not avg:
            # [batch_size, -1, 1, dim]
            ...
            self_neighbor_scores = tf.reduce_mean(self_vectors * neighbor_vectors, axis=-1)
            self_neighbor_scores_normalized = tf.nn.softmax(self_neighbor_scores, dim=-1)

            # [batch_size, -1, n_neighbor, 1]
            self_neighbor_scores_normalized = tf.expand_dims(self_neighbor_scores_normalized, axis=-1)

            # [batch_size, -1, dim]
            neighbors_aggregated = tf.reduce_mean(self_neighbor_scores_normalized * neighbor_vectors, axis=2)
        else:
            # [batch_size, -1, dim]
...
class SumAggregator2(Aggregator):
    def __init__(self, batch_size, dim, dropout=0., act=tf.nn.relu, name=None):
        super(SumAggregator2, self).__init__(batch_size, dim, dropout, act, name)
    ...
    def _call(self, self_vectors, neighbor_vectors):
        # [batch_size, -1, dim]
        neighbors_agg = self._mix_neighbor_vectors(self_vectors, neighbor_vectors)
...
class ConcatAggregator2(Aggregator):
    def __init__(self, batch_size, dim, dropout=0., act=tf.nn.relu, name=None):
        super(ConcatAggregator2, self).__init__(batch_size, dim, dropout, act, name)
...
class NeighborAggregator2(Aggregator):
    def __init__(self, batch_size, dim, dropout=0., act=tf.nn.relu, name=None):
        super(NeighborAggregator2, self).__init__(batch_size, dim, dropout, act, name)
