...
def single_neg_tail_choice(high, size, rel, exclusion=None):
    sampled_neg_tails, i = [], 0
    while size - i:
        sampled_tail = np.random.randint(low=0, high=high, size=1)[0]

        if (sampled_tail, rel) not in exclusion and sampled_tail not in sampled_neg_tails:
            sampled_neg_tails.append(sampled_tail)
            i += 1

    return sampled_neg_tails[0] if size == 1 else sampled_neg_tails