...
def get_info(result: dict=None, epoch=None, auc=None, f1=None, test=True):
    info = ''
    if epoch is not None and auc is not None and f1 is not None:
        if test:
            info += f"epoch {epoch}\ttest auc: {format(auc, '.4f')}\tf1: {format(f1, '.4f')}\n"
        else:
            info += f"epoch {epoch}\ttrain auc: {format(auc, '.4f')}\tf1: {format(f1, '.4f')}\n"
